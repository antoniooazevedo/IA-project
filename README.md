...
- **Random Heuristic**: This heuristic uses a kind of Monte Carlo simulation to bond all atoms together. It randomly samples a sequence of moves and tries to reach the goal state. We then limit the number of moves to four times the number of tiles in the grid. Its only limitation is that it can't move in the same direction that it previously chose. If it doesn't reach the goal state, it will restart the process. This heuristic is used to compare the performance of other heuristics.
